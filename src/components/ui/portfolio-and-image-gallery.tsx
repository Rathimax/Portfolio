...
function useResponsiveValue(baseValue: number, mobileValue: number) {
    const [value, setValue] = useState(() => {
        ...
        return window.innerWidth < 768 ? mobileValue : baseValue;
    });
...
>(
    (
        ...
        useGSAP(
            () => {
                if (!isDesktop || !pinRef.current || !containerRef.current || childrenCount === 0)
                    return;
        ...
        if (!isDesktop) {
            return (
                <div
                    key="mobile-gallery"
                    ref={ref}
                    className={`w-full flex flex-col gap-12 items-center py-10 px-4 ${className}`}
                    {...rest}
                >
                    {childrenNodes.map((child, index) => (
                        ...
                        >
                            <div className={`w-[350px] max-w-full h-[440px] transition-all duration-500 ease-out ${hoveredIndex === index ? 'scale-105' : 'scale-100'}`}>
                                {child}
                            </div>
                        </div>
                    ))}
        ...
        const maxCardHeight = childSize ? childSize.h : 400; // max card size
        const scaleFactor = 1.20; // Matches user's manual change
        const topPaddingRequired = (maxCardHeight * scaleFactor) / 2 + 50; // Accounting for scale-120 + -translate-y-6

        const visibleAreaHeight = circleDiameter * visibleDecimal + topPaddingRequired;
